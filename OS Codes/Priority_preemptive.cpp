...
void priority(struct Process array[], int n)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        totalbursttime += array[i].burstTime;
        printf("%d\t%d\t%d\t\t%d\t\t%d\t%d\n",array[i].processID,array[i].tempburstTime,array[i].arrivalTime,array[i].priority,array[i].tempburstTime+(array[i].outtime - array[i].intime -array[i].tempburstTime),(array[i].outtime - array[i].intime -array[i].tempburstTime));
    }
